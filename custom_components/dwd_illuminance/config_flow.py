...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            await self.async_set_unique_id(f"dwd_illuminance_{user_input[CONF_NAME]}")
            self._abort_if_unique_id_configured()
            return self.async_create_entry(title="DWD Illuminance", data=user_input)

            _LOGGER.debug(
                "Initialized new dwd_illuminance with id: {unique_id}"
            )

        data_schema = vol.Schema(
            {
                vol.Required(CONF_NAME): str,
                vol.Required(CONF_MODE, default="normal"): str,
